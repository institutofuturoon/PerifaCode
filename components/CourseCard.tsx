...
const CategoryIcon: React.FC<{ category?: string }> = ({ category }) => {
    ...
    switch (category) {
        case 'IDIOMA':
            icon = <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
  ...
  return (
    ...
    >
      <div className="overflow-hidden aspect-video relative">
        ...
        {status && (
            <div className={`absolute top-2 left-2 backdrop-blur-sm px-1.5 py-0.5 rounded text-[10px] font-semibold text-white ${status.classes}`}>
              {status.text}
            </div>
        )}
        <div className="absolute top-2 right-2 bg-black/50 backdrop-blur-sm px-1.5 py-0.5 rounded text-[10px] font-semibold text-white">
          {course.skillLevel}
        </div>
      </div>
      <div className="p-4 flex flex-col flex-grow">
        ...
        <h3 className="mt-2 text-base font-bold text-white group-hover:text-[#c4b5fd] transition-colors line-clamp-2 flex-grow">{course.title}</h3>
        <p className="mt-1 text-xs text-gray-400 line-clamp-2">{course.description}</p>
      </div>
    </button>
  );
};
