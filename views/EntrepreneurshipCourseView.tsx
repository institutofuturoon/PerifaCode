...
const InscriptionFormModal: React.FC<InscriptionFormModalProps> = ({ isOpen, onClose, courseName = "Empreendedorismo Digital" }) => {
...
const EntrepreneurshipCourseView: React.FC = () => {
    const { navigate } = useAppContext();
        ...
        <div className="aurora-background text-white">
            <main>
                ...
                <Section className="bg-black/20">
                    <SectionTitle subtitle="Você já sabe construir. Agora, aprenda a criar produtos que as pessoas amam e pagam para usar.">
                        Por que todo Dev Deveria Aprender a Empreender?
                    </SectionTitle>
                    <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
                        <BenefitCard icon="👑" title="Seja seu Próprio Chefe" description="Ganhe autonomia para trabalhar em seus próprios projetos e criar o futuro que você quer, nos seus termos." />
                        <BenefitCard icon="💸" title="Crie Novas Fontes de Renda" description="Transforme suas ideias de apps, SaaS e ferramentas em negócios lucrativos e escaláveis." />
                        <BenefitCard icon="🧠" title="Pense como Dono" description="Desenvolva uma mentalidade de negócios que te fará um dev mais valioso, mesmo em um emprego tradicional." />
                        <BenefitCard icon="🎯" title="Resolva Problemas Reais" description="Aprenda a identificar problemas reais do mercado e a construir soluções que resolvem dores de verdade." />
                    </div>
                </Section>
                
                {/* Curriculum Section */}
                <Section>
                     <SectionTitle subtitle="Uma trilha prática que te ensina o caminho das pedras para criar um negócio digital, do zero ao lançamento.">
                        Da Ideia à Primeira Venda: Sua Jornada Empreendedora
                    </SectionTitle>
                    <div className="max-w-4xl mx-auto bg-white/5 border border-white/10 rounded-2xl p-8 md:p-12">
                        <div className="grid md:grid-cols-2 gap-8">
                            <CurriculumItem title="Da Ideia ao MVP" description="Como validar sua ideia, definir seu público-alvo e construir um Produto Mínimo Viável (MVP) de forma rápida." />
                            <CurriculumItem title="Modelo de Negócios" description="Desenhe seu modelo com Canvas, defina sua proposta de valor e estratégias de monetização." />
                            <CurriculumItem title="No-Code & Low-Code" description="Construa e valide ideias sem escrever uma linha de código, usando ferramentas modernas." />
                            <CurriculumItem title="Marketing Digital para Devs" description="Aprenda o básico de SEO, marketing de conteúdo e como usar as redes sociais para atrair seus primeiros usuários." />
                            <CurriculumItem title="Vendas e Métricas" description="Entenda como fazer sua primeira venda, definir preços e acompanhar as métricas que realmente importam (CAC, LTV)." />
                            <CurriculumItem title="Pitch e Captação" description="Aprenda a apresentar sua ideia de forma convincente para clientes, parceiros e potenciais investidores." />
                        </div>
                    </div>
                </Section>
                ...
                {/* Methodology Section */}
                 <Section className="bg-black/20">
                    <SectionTitle subtitle="Aulas práticas, estudos de caso e mentoria com quem já está no campo de batalha do empreendedorismo.">
                        Nossa Metodologia: Construa seu Negócio, Não Apenas um Projeto
                    </SectionTitle>
                    <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
                        <BenefitCard icon="🚀" title="Aulas Práticas" description="Estudos de caso reais e workshops para aplicar os conceitos diretamente no seu projeto." />
                        <BenefitCard icon="💡" title="Mentoria com Fundadores" description="Receba feedback de empreendedores que já trilharam o caminho e aprenderam com os erros e acertos." />
                        <BenefitCard icon="🎤" title="Pitch Day" description="Apresente seu projeto final para uma banca de mentores e parceiros, simulando um ambiente real." />
                        <BenefitCard icon="🤝" title="Comunidade Empreendedora" description="Conecte-se com outros devs que também estão na jornada de criar seus próprios negócios." />
                    </div>
                </Section>
                ...
                <Section>
                    <div className="text-center bg-grid-pattern p-8 md:p-16 rounded-2xl border border-[#8a4add]/20" style={{backgroundImage: 'radial-gradient(circle at center, #8a4add10, transparent 60%)'}}>
                        <h2 className="text-3xl md:text-4xl font-black text-white">Sua Ideia Merece Sair do Papel.</h2>
                        <p className="mt-4 max-w-2xl mx-auto text-lg text-gray-300">
                           As vagas são limitadas. Inscreva-se para garantir seu interesse na próxima turma e comece a construir o futuro, hoje.
                        </p>
                        <div className="mt-8">
                             <button onClick={() => setIsFormOpen(true)} className="inline-block bg-gradient-to-r from-[#8a4add] to-[#f27983] text-white font-bold py-3 px-8 rounded-lg hover:opacity-90 transition-all duration-300 transform hover:scale-105 shadow-lg shadow-[#8a4add]/30">
                                Inscreva-se Agora
            ...
            <InscriptionFormModal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} courseName="Empreendedorismo Digital" />
        </div>
    );
};

export default EntrepreneurshipCourseView;
