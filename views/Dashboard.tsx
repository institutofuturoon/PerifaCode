...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <div className="mb-12">
        <h1 className="text-4xl font-black text-white">Bem-vinda de volta, <span className="bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-pink-500">{user.name.split(' ')[0]}</span>!</h1>
      ...
      <div className="grid lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2 space-y-8">
            ...
            {inProgressCourses.map(({ course, progress }) => {
              ...
              return (
                <button 
                  ...
                  className="w-full bg-white/5 backdrop-blur-sm p-6 rounded-lg border border-white/10 flex flex-col sm:flex-row items-center gap-6 hover:border-purple-500/50 transition-colors duration-300 text-left"
                >
             ...
             {inProgressCourses.length === 0 && (
              <div className="text-center py-10 bg-white/5 rounded-lg border border-white/10">
                ...
                <button onClick={() => navigate('courses')} className="mt-4 text-purple-400 font-semibold">Explorar cursos</button>
              </div>
            )}
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            {completedCourses.map(course => {
                if(!course) return null;
                return (
                    <div key={course.id} className="bg-white/5 backdrop-blur-sm p-6 rounded-lg border border-white/10 flex items-center gap-4 hover:border-purple-500/50 transition-colors duration-300">
                        ...
                        <div>
                            ...
                            <button onClick={() => navigateToCertificate(course)} className="text-sm text-purple-400 hover:underline">Ver certificado</button>
                        </div>
                    </div>
                )
            })}
             {completedCourses.length === 0 && (
              <div className="text-center py-6 bg-white/5 rounded-lg border border-white/10 col-span-full">
        ...
        <aside className="space-y-8">
            <div className="bg-white/5 backdrop-blur-sm p-6 rounded-lg border border-white/10">
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <p className="text-gray-400 mt-1 text-sm">{user.xp} XP</p>
                    </div>
                    <div className="text-center">
                        <p className="text-sm text-gray-400">Nível</p>
                        <p className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-pink-500">{userLevel}</p>
                    </div>
                </div>
                <div className="mt-2">
             ...
             <div className="bg-white/5 backdrop-blur-sm p-6 rounded-lg border border-white/10 flex items-center gap-4">
            ...
            <div className="bg-white/5 backdrop-blur-sm p-6 rounded-lg border border-white/10">
                ...
                <p className="text-gray-400 mt-2 text-sm">Conecte-se com outros alunos e mentores. Tire dúvidas, compartilhe projetos e cresça em conjunto.</p>
                <a href="#" className="mt-4 block w-full text-center bg-purple-600 text-white font-semibold py-2 rounded-lg hover:bg-purple-700 transition-colors duration-300 shadow-lg shadow-purple-500/20 hover:shadow-purple-500/40">
                    Acessar Discord
                </a>
            </div>
        </aside>
