    ...
    const Overview = () => (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid lg:grid-cols-3 gap-6">
                {/* Course Performance */}
                <div className="lg:col-span-2 bg-white/[0.02] rounded-xl border border-white/10 p-6">
    ...
    const OverviewContent = () => {
        return (
            <div className="space-y-8">
                {/* Simplified Hero Profile Section */}
                <div className="relative overflow-hidden rounded-3xl bg-[#121214] border border-white/10 p-8">
                    {/* Background effects */}
                    <div className="absolute top-0 right-0 w-96 h-96 bg-[#8a4add]/10 rounded-full blur-[100px] -mr-20 -mt-20 pointer-events-none"></div>
                    
                    <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
                        {/* Avatar Simple */}
                        <div className="w-24 h-24 rounded-full p-1 bg-white/10">
                            <img src={user.avatarUrl} className="w-full h-full rounded-full object-cover" alt={user.name} />
                        </div>
    
                        <div className="text-center md:text-left flex-1">
                            <h2 className="text-3xl font-black text-white mb-1">Olá, {user.name.split(' ')[0]}! 👋</h2>
                            <p className="text-gray-400 text-sm">Continue sua jornada de aprendizado.</p>
                        </div>
    
                        {/* Simplified Stats */}
                        <div className="flex flex-wrap gap-4 justify-center">
                            <div className="bg-white/5 border border-white/10 rounded-2xl p-4 text-center min-w-[100px] hover:bg-white/10 transition-colors">
                                <div className="text-2xl mb-1">⚡</div>
                                <div className="font-black text-yellow-400 text-xl">{user.xp || 0}</div>
                                <div className="text-[10px] text-gray-500 uppercase font-bold">Total XP</div>
                            </div>
                            <div className="bg-white/5 border border-white/10 rounded-2xl p-4 text-center min-w-[100px] hover:bg-white/10 transition-colors">
                                <div className="text-2xl mb-1">🔥</div>
                                <div className="font-black text-orange-400 text-xl">{user.streak || 0}</div>
                                <div className="text-[10px] text-gray-500 uppercase font-bold">Dias Seguidos</div>
                            </div>
                            <div className="bg-white/5 border border-white/10 rounded-2xl p-4 text-center min-w-[100px] hover:bg-white/10 transition-colors">
                                <div className="text-2xl mb-1">🏆</div>
                                <div className="font-black text-purple-400 text-xl">{user.achievements?.length || 0}</div>
                                <div className="text-[10px] text-gray-500 uppercase font-bold">Conquistas</div>
                            </div>
                        </div>
                    </div>
                </div>
    
                {/* Current Mission (Continue Learning) - IMPROVED UI */}
                {latestInProgress && nextLesson ? (
                    <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-black group shadow-2xl shadow-black/50">
                        
                        {/* Background Image with Heavy Overlay */}
                        <div className="absolute inset-0 z-0">
                            <img 
                                src={latestInProgress.course.imageUrl} 
                        ...
                        <div className="relative z-10 p-8 md:p-10 flex flex-col md:flex-row items-end gap-8">
                            <div className="flex-1">
                                <div className="flex items-center gap-3 mb-3">
                                    <span className="inline-block py-1 px-3 rounded bg-[#8a4add]/20 text-[#c4b5fd] text-[10px] font-bold uppercase tracking-widest border border-[#8a4add]/30">
                                        {latestInProgress.course.track}
                                    </span>
                                ...
                                <h2 className="text-xl text-gray-300 font-medium mb-1">{latestInProgress.course.title}</h2>
                                ...
                                <div className="flex items-center gap-6">
                                    <button 
                                        ...
                                        className="flex items-center gap-3 bg-white text-black font-bold py-3.5 px-8 rounded-full hover:bg-[#c4b5fd] hover:scale-105 transition-all duration-300 shadow-[0_0_20px_rgba(255,255,255,0.3)] group/btn"
                                    >
                                    ...
                                    <div className="flex flex-col min-w-[120px]">
                                        <div className="flex justify-between text-xs font-bold text-gray-400 mb-1.5">
                                            <span>PROGRESSO</span>
                                            <span className="text-white">{latestInProgress.progress}%</span>
                                        </div>
                                        <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden">
                                            <div 
                                                className="h-full bg-gradient-to-r from-[#8a4add] to-[#c4b5fd] rounded-full" 
                            ...
                            {/* Floating Play Icon for Decor */}
                            <div className="hidden md:flex w-24 h-24 rounded-full border-2 border-white/10 items-center justify-center bg-white/5 backdrop-blur-sm shadow-2xl group-hover:bg-white/10 transition-colors">
                                <svg className="w-10 h-10 text-white opacity-80" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
                            </div>
                        </div>
                    </div>
                ) : (
                ...
                <div className="space-y-6 pb-12">
                     <div className="flex items-center justify-between">
                        <h3 className="text-xl font-bold text-white">Meus Treinamentos</h3>
                     </div>
                     
                     <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {allMyCourses.length > 0 && allMyCourses.map((course) => (
                            <CourseCard 
                                key={course.id} 
                                course={course} 
                        ...
                        <button 
                            ...
                            className="group flex flex-col items-center justify-center h-full min-h-[300px] rounded-xl border border-dashed border-white/10 hover:border-[#8a4add]/50 hover:bg-[#8a4add]/5 transition-all"
                        >
                            <div className="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform group-hover:bg-[#8a4add] group-hover:text-white text-gray-500">
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
                            </div>
                            <span className="text-gray-400 font-bold group-hover:text-white transition-colors">Explorar Novos Cursos</span>
                        </button>
                    </div>
                </div>
            </div>
