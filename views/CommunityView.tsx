...
const EmptyState: React.FC<{ type: 'project' | 'forum', onClear: () => void }> = ({ type, onClear }) => (
    <div className="text-center py-16 bg-white/5 rounded-xl border border-white/10 border-dashed flex flex-col items-center justify-center animate-fade-in">
        ...
        <p className="text-gray-300 text-base font-medium">Nenhum resultado encontrado.</p>
        <p className="text-gray-500 text-xs mt-1 max-w-md">Não encontramos nada com os filtros atuais. Tente buscar por outro termo ou limpe os filtros.</p>
...
const CommunityView: React.FC = () => {
    ...
    const TabButton: React.FC<{ tab: ActiveTab; label: string; count?: number }> = ({ tab, label, count }) => (
        <button
            ...
            className={`relative px-5 py-3 text-sm font-bold transition-all duration-300 flex items-center gap-2 ${
                activeTab === tab 
        ...
        >
            {label}
            {count !== undefined && (
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${activeTab === tab ? 'bg-[#8a4add] text-white' : 'bg-white/10 text-gray-500'}`}>
                    {count}
                </span>
            )}
        </button>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-10">
                ...
                <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
                    {user && (
                        <button
                            ...
                            className="bg-gradient-to-r from-[#6d28d9] to-[#8a4add] text-white font-bold py-2.5 px-6 rounded-lg hover:opacity-90 transition-all duration-300 shadow-lg shadow-[#8a4add]/30 whitespace-nowrap flex items-center justify-center gap-2 transform hover:scale-105 text-sm"
                        >
            ...
            <div className="grid lg:grid-cols-4 gap-6 relative">
                ...
                <div className="lg:col-span-3">
                    {/* Tabs & Search Wrapper */}
                    <div className="sticky top-16 z-30 bg-[#09090B]/95 backdrop-blur-xl border-b border-white/10 -mx-4 px-4 sm:mx-0 sm:px-0 sm:rounded-t-xl mb-6 pt-2">
                        <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                            ...
                            <div className="relative w-full sm:w-64 mb-2 sm:mb-0 sm:mr-2">
                                <input 
                                    ...
                                    className="w-full p-2 pl-8 bg-white/10 rounded-md border border-white/10 focus:ring-1 focus:ring-[#8a4add] focus:outline-none transition-colors text-xs text-white placeholder-gray-500"
                                />
                                <div className="absolute inset-y-0 left-0 flex items-center pl-2.5 pointer-events-none">
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="mb-6 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 flex gap-2 no-scrollbar">
                        {filters.map(filter => (
                            <button
                                key={filter}
                                onClick={() => setActiveFilter(filter)}
                                className={`flex-shrink-0 px-3 py-1 text-xs font-semibold rounded-full transition-all duration-300 border ${
                                activeFilter === filter
                    ...
                    <div className="min-h-[300px]">
                        {activeTab === 'projects' && (
                            <div className="grid sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4 animate-fade-in">
                ...
                <aside className="lg:col-span-1 space-y-6">
                    <div className="lg:sticky lg:top-24 space-y-6">
                        ...
                        <div className="p-5 bg-gradient-to-br from-[#18181b] to-[#09090b] rounded-xl border border-white/10 text-center relative overflow-hidden group">
                            ...
                            <div className="relative z-10">
                                ...
                                <h3 className="text-base font-bold text-white mb-1">Quer colaborar?</h3>
                                ...
                                <button 
                                    ...
                                    className="w-full py-2 bg-white/10 border border-white/20 text-white font-bold rounded-lg hover:bg-white/20 hover:border-white/30 transition-all duration-300 text-xs"
                                >
                                    Explorar Projetos
                                </button>
                            </div>
